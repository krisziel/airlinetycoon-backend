...
describe 'airtycoon API -- messages#' do
  ...
  xit 'can receive a list of all conversations' do
  ...
  xit 'can create a new conversation' do
  ...
  xit 'cannot access a conversation of another user' do
  ...
  xit 'returns the existing conversation if conversation combo already exists' do
    Conversation.create(sender_id:1,recipient_id:2)
    post 'chat/conversation',
    {
      conversation:{
  ...
  xit 'can receive messages from a specific user' do
  ...
  xit 'can receive messages since a given time' do
    conv = Conversation.new(sender_id:3,recipient_id:1)
    conv.save
    conv.messages.create(airline_id:1,body:"hello")
    conv.messages.create(airline_id:3,body:"hi")
  ...
  xit 'can load messages with offset' do
    conv = Conversation.new(sender_id:3,recipient_id:1)
    conv.save
    conv.messages.create(airline_id:1,body:"hello")
    conv.messages.create(airline_id:3,body:"hi")
  ...
  xit 'cannot send duplicate messages within fifteen seconds' do
    conv = Conversation.new(sender_id:3,recipient_id:1)
    conv.save
    conv.messages.create(airline_id:1,body:"hello")
  ...
  xit 'cannot send an empty message' do
    conv = Conversation.new(sender_id:3,recipient_id:1)
    conv.save
    post 'chat/conversation/1/message',
    {
  ...
  xit 'can send multiple non-duplicate messages' do
    conv = Conversation.new(sender_id:1,recipient_id:4)
    conv.save
    post 'chat/conversation/1/message',
    {
